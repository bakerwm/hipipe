...
def p7_demx_se(fn1, smp_list, path_out, n_mismatch=0, bc_in_read=1, n_left=3,
    ...
    with fq_reader(fn1, 'rt') as fi:
        for s in SeqIO.parse(fi, 'fastq'):
            s_new, p7_query, bc_query = seq_index_finder(s, cut=cut, 
                bc_len=0, bc_n_left=0, bc_n_right=0)
            tag = seq_validator(p7_query, p7_dict, mm=n_mismatch)
            if tag is None: #undemx
                tag = 'undemx'
            SeqIO.write(s_new, p7_writer[tag], 'fastq')
...
def p7_demx_pe(fn1, fn2, smp_list, path_out, n_mismatch=0, bc_in_read=1, 
    ...
    with fq_reader1(fn1, 'rt') as f1, fq_reader2(fn2, 'rt') as f2:
        ss1 = SeqIO.parse(f1, 'fastq')
        ss2 = SeqIO.parse(f2, 'fastq')
        while True:
            try:
                s1 = next(ss1)
                s2 = next(ss2)
                # check read names from read1
                if not s1.name == s2.name:
                    raise ValueError(s1.name + '\n' + s2.name)
                # default bc in s1
                ...
                if tag is None: #undemx
                    tag = 'undemx'
                SeqIO.write(s1_new, p7_writer[tag][0], 'fastq')
                SeqIO.write(s2, p7_writer[tag][1], 'fastq')
                p7_count[tag]['count'] += 1
...
def bc_demx_se(fn1, smp_list, path_out, n_mismatch=0, bc_in_read=1, 
    ...
    with fq_reader(fn1, 'rt') as fi:
        for s in SeqIO.parse(fi, 'fastq'):
            ...
            if tag is None: #undemx
                tag = 'undemx'
            SeqIO.write(s_new, bc_writer[tag], 'fastq')
    ...
    with fq_reader1(fn1, 'rt') as f1, fq_reader2(fn2, 'rt') as f2:
        ...
        while True:
            try:
                ...
                if tag is None: #undemx
                    tag = 'undemx'
                SeqIO.write(s1_new, bc_writer[tag][0], 'fastq')
                SeqIO.write(s2, bc_writer[tag][1], 'fastq')
                bc_count[tag]['count'] += 1
            except StopIteration:
                break
    # close writers
...
def p7_bc_demx_se(fn1, smp_list, path_out, n_mismatch=0, bc_in_read=1,
    ...
    with fq_reader(fn1, 'rt') as fi:
        for s in SeqIO.parse(fi, 'fastq'):
            s_new, p7_query, bc_query = seq_index_finder(s, cut=cut, 
                bc_len=bc_len, bc_n_left=n_left, bc_n_right=n_right)
            tag1 = seq_validator(p7_query, p7_dict, mm=n_mismatch)
            tag2 = seq_validator(bc_query, bc_dict, mm=n_mismatch)
            if tag1 == 'multi' or tag2 == 'multi':
                tag = 'multi'
            elif tag1 is None and not tag2 is None and 'NULL,' + tag2 in p7_bc_dict:
                tag = 'NULL,' + tag2
            elif tag2 is None and not tag1 is None and tag1 + ',NULL' in p7_bc_dict:
                tag = tag1 + ',NULL'
            else:
                tag = 'undemx'
            SeqIO.write(s_new, p7_bc_writer[tag], 'fastq')
        ...
        while True:
            try:
                ...
                if tag1 == 'multi' or tag2 == 'multi':
                    ...
                    tag = 'undemx'
                SeqIO.write(s1_new, p7_bc_writer[tag][0], 'fastq')
                SeqIO.write(s2, p7_bc_writer[tag][1], 'fastq')
                p7_bc_count[tag]['count'] += 1
            except StopIteration:
                break
    # close writers
